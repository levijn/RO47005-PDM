...
class Dubins:
    ...
    def dubins_path(self, start, end, return_distance=False):
        """
        ...
        return self.generate_points(start, end, dubins_path, straight, return_distance)

    def generate_points(self, start, end, dubins_path, straight, return_distance=False):
        """
        ...
        """
        if straight:
            return self.generate_points_straight(start, end, dubins_path, return_distance)
        return self.generate_points_curve(start, end, dubins_path, return_distance)
    ...
    def lsl(self, start, end, center_0, center_2):
        ...
        """
        straight_dist = self.dist(center_0, center_2)
    ...
    def lrl(self, start, end, center_0, center_2):
        ...
        """
        dist_intercenter = self.dist(center_0, center_2)
    ...
    def generate_points_straight(self, start, end, path, return_distance=False):
        """
        ...
        dist_straight = self.dist(ini, fin)
        ...
        if return_distance:
            return np.array(points), total
        
        return np.array(points)

    def generate_points_curve(self, start, end, path, return_distance=False):
        """
        ...
        center_0 = self.find_center(start, 'L' if path[0] > 0 else 'R')
        center_2 = self.find_center(end, 'L' if path[1] > 0 else 'R')
        intercenter = self.dist(center_0, center_2)
        center_1 = (center_0 + center_2)/2 +\
                   np.sign(path[0])*self.ortho((center_2-center_0)/intercenter)\
